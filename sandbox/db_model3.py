...
class Product(Base):
    ...
    product_company_associations: Mapped[Dict[str, ProductCompanyAssociation]] = relationship(
        ...
        collection_class=attribute_keyed_dict("role"),
        cascade="all, delete-orphan",
    )
    # the same 'product_company_associations'->'company' proxy as in
    # the basic dictionary example.
    companies: AssociationProxy[Dict[str, str]] = association_proxy(
        "product_company_associations",
        "company",
        creator=lambda k, v: ProductCompanyAssociation(role=k, company=v),
    )
    # idproduct = Column(Integer, primary_key=True)
    # name = Column(String(100), unique=True)

    # #product_comp_associations = relationship("company_product_ass", back_populates="product", cascade="all, delete-orphan")
    # # association proxy of "user_keyword_associations" collection
    # # to "keyword" attribute
    # companies = association_proxy(
    #     "product_comp_associations",
    #     "company",
    #     creator=lambda company_obj, role: CompanyProductAss(company=company_obj, role=role),
    # )
...
#     company_prod_associations= relationship( # : Mapped[Dict[str, CompanyProductAss]] 
#         back_populates="company", 
#         collection_class=attribute_keyed_dict("role"),
#         cascade="all, delete-orphan")
#     # association proxy of "user_keyword_associations" collection
#     # to "keyword" attribute
#     products: AssociationProxy[Dict[str, Product]] = association_proxy(
#         "company_prod_associations",
#         "product",
#         creator=lambda prod_obj, role: CompanyProductAss(role=role, product=prod_obj)
#     )
...
# class CompanyProductAss(Base):
#     __tablename__ = "company_product_ass"

#     idcompany = Column(Integer, ForeignKey('company.idcompany'), primary_key=True)
#     idproduct = Column(Integer, ForeignKey('product.idproduct'), primary_key=True)
#     role = Column(String(45), nullable=False)

#     company = relationship("Company", back_populates="company_prod_associations")
#     product = relationship("Product", back_populates="product_comp_associations")
...
class HTAAgency(Base):
    ...
    personnel = relationship("Personal", back_populates="agency")
...
class HTADocument(Base):
    ...
    personnel = relationship("Personal", secondary="HTA_document_has_personal", back_populates="HTA_documents")
    indications = relationship("Indication", secondary="HTA_document_has_indication", back_populates="HTA_documents")
...
class HTADocumentHasIndication(Base):
    __tablename__ = 'HTA_document_has_indication'
...
class HTADocumentHasPersonal(Base):
    __tablename__ = 'HTA_document_has_personal'

    idHTA_document = Column(Integer, ForeignKey('HTA_document.idHTA_document'), primary_key=True)
    idpersonal = Column(Integer, ForeignKey('personal.idpersonal'), primary_key=True)
...
class Indication(Base):
    ...
    HTA_documents = relationship("HTADocument", secondary="HTA_document_has_indication", back_populates="indications")
...
class Personal(Base):
    ...
    agency = relationship("HTAAgency", back_populates="personnel")
    HTA_documents = relationship("HTADocument", secondary="HTA_document_has_personal", back_populates="personnel")
...
class Trial(Base):
    ...
    HTA_document = relationship("HTADocument", back_populates="trials")
