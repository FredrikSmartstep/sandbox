...
from pydantic_models import HTA_Document_Basis, PICO, HTA_Document_Staff, Team, PICOs_Partial_Agency, PICOs_Partial_Company,\
    PICO_Analysis_Cost_Company, PICO_Analysis_Cost_Agency, Costs, Panel, PICOs, PICOs_comp, PICOs_ag, Analysis_List_Comp, Analysis_List_Ag, \
        Analysis_List, Trials, References, HTA_Document_Extend, Panel2
...
class Worker_dossier:
    ...
    def __init__(self, client, doc_type = 'dossier', model = 'gpt-4o'):
    ...
    def get_ICD(self, res):
        ...
        for ind in res['indications']:
            ...
            icd10_code = self.get_response(self.get_ICD_code(ind['indication']))
    ...
    def initial_parsing(self):
        ...
        res = self.convert_to_pydantic(res_json_pre, HTA_Document_Basis)
    ...
    def parse_file_2(self, file_dir):
        ...
        try:
            ...
            self.decision_document_title = ntpath.basename(file_paths[0])
            # -----------------------------------------------
            # The decision file
            # -----------------------------------------------
            # Add vector store and file
            self.add_decision_file(file_paths[0])
            self.create_auto_vector_store(self.decision_document_title)
            # Do the basic parsing
            self.create_original_message_2()
            self.create_thread()
            attempt = 0
            res = None
            ...
            if not res:
                log.error('Unable to parse file ' + self.decision_document_title)
                return None
            # Update with correct title
            res['title'] =  self.decision_document_title  
            ...
            if len(res['staff'])<2: 
                ...
                #res['staff'] = res_staff_json#['staff'] This is wrong. It adds an each layer of 'staff'
            ...
            if len(file_paths)>1:
                ...
                res = self.parse(res, self.create_basis_message(), HTA_Document_Extend)
                ...
                log.info('Missing basis document for ' + self.decision_document_title)
            ...
            res_sum = self.convert_to_pydantic(ass_results_sum, HTA_Document_Basis)
            ...
            if len(self.message_files)>1: # TODO Finer chunking for results chapter
                ...
                log.info('Lacking chapters for file {}'.format(self.decision_document_title))
            ...
            pico_list = [int(x['pico_nr'])-1 for x in self.json_parse(ass_results_an)['analyses']] # need to start at 0
            res_analysis_comp = self.convert_to_pydantic(ass_results_an, Analysis_List_Comp)
    ...
    def convert_to_pydantic(self, input, format):
        ...
        while attempt<MAX_NR_OF_RETRIES:
            try:
                ...
                attempt = attempt + 1
                time.sleep(attempt)
                log.error('Pydantic parsing failed for file {}'.format(self.decision_document_title), exc_info=sys.exc_info())
        return completion#.choices[0].message
    ...
    def add_basis_files(self, file_paths):
        log.info('Adding basis files')

        for fp in file_paths[1:]: # Skipping the first as that is the decision file
    ...
    def create_basis_message(self):
        ...
        return [
            ...
            "content": 
                ...
                from the file named '{}'. If you cannot find the file, return title as 'not found' and the rest of the requested output as blanks. 
                Information that you cannot find should be returned as blank in the corresponding field.
                Respond in a plain JSON format without any Markdown or code block formatting in english""".format(self.message_files[1]['filename']),
            }
        ]
    ...
    def run_parsing(self, additional_messages=None, additional_instructions=None):
        ...
        while attempt<MAX_NR_OF_RETRIES:
            try:
                ...
                attempt = attempt + 1
                time.sleep(attempt)
                log.error('OpenAI streaming failed for file {} with message {}'.format(self.decision_document_title, additional_messages), exc_info=sys.exc_info())
                 # Let's kill that run
                runs = self.client.beta.threads.runs.list(self.thread.id)
                ...
                if (run.status!='cancelled') & (run.status!='completed') & (run.status!='failed') & (run.status!='expired'):
                    ...
                    if (run.status!='cancelled') | (run.status!='completed') | (run.status!='failed') | (run.status!='expired'):
                        log.error('Unsuccesful in canceling failing parsing for file {} with message {}'.format(self.decision_document_title, additional_messages)) #https://community.openai.com/t/assistant-api-cancelling-a-run-wait-until-expired-for-several-minutes/544100/4
                        # This is flimsy
    ...
    def get_results(self):
        ...
        while attempt<MAX_NR_OF_RETRIES:
            try:
                ...
                attempt = attempt + 1
                time.sleep(attempt)
                log.error('Failed to get results from OpenAI parsing for file ().'.format(self.decision_document_title), exc_info=sys.exc_info())
                log.info('Trying again. Attempt: ' + str(attempt))
