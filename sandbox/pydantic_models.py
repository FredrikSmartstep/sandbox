...
# the validation set is used to ensure that data was indeeed extracted by openAI by checking thatit is populated
...
class Trials(OurBaseModel):
    trials: conlist(item_type=Trial, min_length=)
...
class Demographics(OurBaseModel):
    ...
    # @classmethod
    # def transform_gender(cls, raw: str) -> str:
    #     raw = re.sub(r'both','All', raw, flags=re.I)
    #     return raw
